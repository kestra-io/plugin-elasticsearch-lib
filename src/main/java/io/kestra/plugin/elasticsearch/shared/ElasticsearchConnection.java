...
/**
 * Shared Elasticsearch connection, consumed by both {@code plugin-elasticsearch} (OSS) and
 * {@code plugin-ee-elasticsearch} (EE). Any change here affects both plugins' {@code connection.*}
...
public class ElasticsearchConnection {
    ...
    @Schema(
        ...
        description = "**WARNING — SECURITY RISK**: When enabled, disables BOTH TLS certificate validation " +
            "(`TrustAllStrategy`) AND hostname verification (`NoopHostnameVerifier`). This makes the connection " +
            "vulnerable to man-in-the-middle attacks and must never be used in production. " +
            "Prefer configuring a proper truststore for self-signed certificates instead."
    )
    @PluginProperty(group = "advanced")
    ...
    public Rest5Client client(RunContext runContext) throws IllegalVariableEvaluationException {
